...
const withRetry = async <T>(fn: () => Promise<T>, retries = 3, delay = 500): Promise<T> => {
  ...
  throw new Error("Exceeded retry limit for MongoDB write.");
};
...
const seed = async () => {
  ...
  for (const category of categories) {
    ...
    await sleep(200); // pause between parent and child writes
    ...
    for (const subCategory of category.subcategories || []) {
      ...
      await sleep(100); // pause between child category writes
    }
  }
};
